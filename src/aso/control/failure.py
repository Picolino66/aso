"""Roteamento de falha (fluxo §13, Princípio central) — ADR-0019.
...
# Verificação nomeada da bateria (fluxo §12, ADR-0022): falha trivial (formatação/lint)
...
# QA manual reprovou (fluxo §16/§17, ADR-0025) — não é a mesma coisa que teste automatizado
...
# Categoria da verificação (fluxo §12, ADR-0022) -> diagnóstico direto. Com a bateria
...
_CATEGORIA_DIAGNOSTICO: dict[str, str] = {
    ...
    # QA manual (fluxo §16/§17, ADR-0025) não vem da bateria nomeada (ADR-0022), mas
...
class FailureRecord(BaseModel):
    """O que o fluxo §13 manda registrar quando algo falha — um item do ring `card.failures`.

    Texto vindo do agente (mensagem, saída, comando) é redigido na construção (ADR-0080, regra 9):
    o ring é persistido no card e servido pela API."""
    ...
    executor: str = ""
    effort: str = ""
    # Nome/categoria da verificação da bateria que falhou (fluxo §12, ADR-0022) — vazio
    ...
    def _redigir(self) -> FailureRecord:
        """Redação no ponto de entrada: qualquer caminho novo que grave falha já vem limpo."""
        for campo in ("comando", "mensagem", "saida"):
            valor = getattr(self, campo)
            if valor:
                object.__setattr__(self, campo, mascarar_segredos(valor))
        return self
...
def diagnosticar(record: FailureRecord) -> str:
    """Classifica a falha, preferindo o fato à heurística (wf §4.3, ADR-0022).

    Com `categoria` preenchida (a falha veio de uma verificação nomeada da
    bateria, fluxo §12), o diagnóstico é direto — fato, não palpite. Só cai na
    heurística por palavras-chave (— frágil e fora de escopo trocar
...
def confianca_diagnostico(record: FailureRecord) -> str:
    ...
    percentual: `record.categoria` preenchida é fato (veio da bateria nomeada, fluxo §12),
    confiança "alta"; caindo na heurística por palavra-chave (wf §4.3), confiança
...
# Tabela de política (wf §4.1 da ADR-0019): diagnóstico -> passos, na ordem em que se
...
_TABELA: dict[str, tuple[str, ...]] = {
    ...
    # Falha trivial (formatação/lint, ADR-0019): nunca sobe effort — o
    ...
    # QA reprovado (fluxo §17, ADR-0025): mais paciente que um teste automatizado —
