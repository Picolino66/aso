...
#
# Para usar agentes CLI REAIS (Claude Code, Codex, Aider), marque os perfis do catálogo como
# candidatos (⚙ Config → "candidato da corrida", ADR-0076). Sem catálogo salvo, a variável
#   export ASO_CANDIDATE_COMMANDS='[{"id":"claude","command":"claude -p"},{"id":"codex","command":"codex exec"}]'
# ainda semeia esses perfis no boot da API (docker compose ou uvicorn).
#
