...
### ExecutionPlan
`id, orchestration_id, execution_mode, strategy(single_agent|sequential_agents|parallel_agents), reason, risk_level(low|medium|high|critical), requires_human_approval, agents(list<PlannedAgent>), success_criteria, fallback_strategy, created_at`
`PlannedAgent = { agent, role, reason, depends_on }`
...
### Agent / AgentRun
`Agent = { id, role(§15), capabilities[], default_executor(llm_provider|cli_agent), context_sections[], reservado, created_at }`
...
### Conflict  (§20)
`id, orchestration_id, type(ARCHITECTURE_CONFLICT|CONTRACT_CONFLICT|DATA_MODEL_CONFLICT|SNAPSHOT_LOCK_CONFLICT|TOOL_PERMISSION_CONFLICT|AGENT_OUTPUT_CONFLICT), source_patch_ids[], description, resolution, status(open|resolved|escalated), created_at`
